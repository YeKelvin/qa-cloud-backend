...
def select_by_loginname(login_name) -> TUserLoginInfo:
    return TUserLoginInfo.query_by(LOGIN_NAME=login_name).first()


def select_all_by_userno(user_no) -> List[TUserLoginInfo]:
    return TUserLoginInfo.query_by(USER_NO=user_no).all()


def delete_all_by_userno(user_no):
    entities = TUserLoginInfo.query_by(USER_NO=user_no).all()
